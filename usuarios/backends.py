    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                # Obtener el rol del empleado (con fallback si la columna no existe)
                try:
                    cursor.execute("SELECT rol FROM usuarios WHERE id_pk = %s LIMIT 1", [emp_id])
                    row = cursor.fetchone()
                    rol_empleado = row[0] if row else 'Administrador'
                except Exception:
                    rol_empleado = 'Administrador'
                    logger.warning("No se pudo obtener columna 'rol' - se asigna Administrador por defecto")
                ...
                # 4. Verificar que el rol tenga acceso al sistema
                roles_acceso = ['Administrador', 'Seguridad']
                if not rol_empleado or rol_empleado not in roles_acceso:
                    logger.warning(f"Acceso denegado para ID {user_id} - rol '{rol_empleado}' no tiene permisos de sistema")
                    if request:
                        messages.error(request, 'Este tipo de usuarios no tiene acceso al sistema')
                    return None
                
                logger.info(f"Autenticación exitosa: ID {user_id} - {nombres} {apellidos}")
                
                # 6. Crear o obtener usuario de Django
                ...
                # 7. Guardar datos adicionales en la sesión
                if request:
                    ...
                    request.session['empleado_rol'] = rol_empleado or ''
