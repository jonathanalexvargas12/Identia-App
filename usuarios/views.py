# usuarios/views.py (VERSIÓN LIMPIA CON BITÁCORA Y NOTIFICACIONES INTEGRADAS)
"""
...
# ==============================================
try:
    from notificaciones.utils import registrar_acceso, registrar_crud_action, get_client_ip
    BITACORA_DISPONIBLE = True
except ImportError:
    # Si el módulo no existe aún, crear funciones dummy
    BITACORA_DISPONIBLE = False
    def registrar_acceso(request, usuario_id, tipo_acceso): return None
    ...
    print("ADVERTENCIA: Módulo notificaciones no disponible. Bitácora desactivada.")
...
def login_view(request):
    ...
    # Verificar si viene de cambio de contraseña
    if request.GET.get('password_changed') == '1':
        messages.success(request, 'Contraseña cambiada correctamente. Por favor, inicie sesión con su nueva contraseña.')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if empleado_id and BITACORA_DISPONIBLE:
                registrar_acceso(request, empleado_id, 'login')
...
def logout_view(request):
    ...
    # ==============================================
    # BITÁCORA: Registrar cierre de sesión (ANTES de hacer logout)
    # ==============================================
    if empleado_id and BITACORA_DISPONIBLE:
        registrar_acceso(request, empleado_id, 'logout')
...
def home_principal_view(request):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT COUNT(*) FROM usuarios WHERE LOWER(estado) = 'activo'")
        ...
        stats = {'total_empleados': 0, 'empleados_hoy': 0}
...
def perfil_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                # ==============================================
                # BITÁCORA: Registrar actualización de perfil
                # ==============================================
                if BITACORA_DISPONIBLE:
                    registrar_crud_action(
                        request,
                        empleado_id,
                        'ACTUALIZAR',
                        'PERFIL',
                        objeto_descripcion=f"Teléfono: {telefono}, Correo: {correo}"
                    )
    ...
    context = {
        'perfil': perfil_data,
        'empleado': {
            'nombres': perfil_data.get('nombres', ''),
            'apellidos': perfil_data.get('apellidos', ''),
...
def cambiar_password_view(request):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT password FROM usuarios_administrativos WHERE id_usuario_fk = %s", [empleado_id])
            resultado = cursor.fetchone()
            
            if not resultado:
                return JsonResponse({'success': False, 'error': 'Usuario no encontrado'}, status=404)
            
            nueva_password_hash = make_password(nueva_password)
            
            cursor.execute("""
                UPDATE usuarios_administrativos 
                SET password = %s 
                WHERE id_usuario_fk = %s
            """, [nueva_password_hash, empleado_id])
            connection.commit()
            
            logger.info(f"Contraseña cambiada para empleado ID: {empleado_id}")
        ...
        return JsonResponse({'success': False, 'error': 'Error al cambiar la contraseña'}, status=500)
...
def gestion_tipos_personal_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            if accion_post == 'agregar':
                ...
                # ==============================================
                # BITÁCORA: Registrar creación de tipo
                # ==============================================
                empleado_id = request.session.get('empleado_id')
                if empleado_id and BITACORA_DISPONIBLE:
                    registrar_crud_action(
                        ...
                        'AGREGAR',
                        'TIPOS_PERSONAL',
                        objeto_descripcion=f"Tipo: {nuevo_nombre}"
                    )
                ...
                # ==============================================
                # BITÁCORA: Registrar edición de tipo
                # ==============================================
                empleado_id = request.session.get('empleado_id')
                if empleado_id and BITACORA_DISPONIBLE:
                    registrar_crud_action(
                        ...
                        'EDITAR',
                        'TIPOS_PERSONAL',
                        objeto_id=id_tipo_original,
                        objeto_descripcion=f"Tipo actualizado de '{id_tipo_original}' a '{nuevo_nombre}'"
                    )
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if tiene_usuarios or tiene_horarios:
                    ...
                    # ==============================================
                    # BITÁCORA: Registrar eliminación de tipo
                    # ==============================================
                    empleado_id = request.session.get('empleado_id')
                    if empleado_id and BITACORA_DISPONIBLE:
                        registrar_crud_action(
                            ...
                            'ELIMINAR',
                            'TIPOS_PERSONAL',
                            objeto_descripcion=f"Tipo eliminado: {id_tipo}"
                        )
    ...
    paginator = Paginator(tipos_personal, registros_por_pagina)
    page = request.GET.get('page', 1)

    try:
        tipos_paginados = paginator.page(page)
    except PageNotAnInteger:
        tipos_paginados = paginator.page(1)
    except EmptyPage:
        tipos_paginados = paginator.page(paginator.num_pages)

    # Calcular información de paginación
    if tipos_paginados:
        inicio = (tipos_paginados.number - 1) * registros_por_pagina + 1
        fin = inicio + len(tipos_paginados.object_list) - 1
        rango_mostrado = f"{inicio}-{fin}"
    else:
        rango_mostrado = "0-0"

    total_registros = len(tipos_personal)
    ...
    context = {
        ...
        'menu_activo': 'seguridad',
        'tipos_personal': tipos_paginados,
        ...
        'pagina_actual': tipos_paginados.number,
...
def gestion_horarios_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            if accion_post == 'agregar':
                ...
                # ==============================================
                # BITÁCORA: Registrar creación de horario
                # ==============================================
                empleado_id = request.session.get('empleado_id')
                if empleado_id and BITACORA_DISPONIBLE:
                    fecha_str = fecha if fecha else 'general'
                    registrar_crud_action(
                        request,
                        empleado_id,
                        'AGREGAR',
                        'HORARIOS',
                        objeto_descripcion=f"Horario para {tipo_personal} ({fecha_str})"
                    )
                ...
                # ==============================================
                # BITÁCORA: Registrar edición de horario
                # ==============================================
                empleado_id = request.session.get('empleado_id')
                if empleado_id and BITACORA_DISPONIBLE:
                    fecha_str = fecha if fecha else 'general'
    ...
    # ========== MANEJO DE ELIMINACIÓN ==========
        ...
        try:
            ...
            # Obtener información del horario antes de eliminarlo (para la bitácora)
            tipo_horario = None
            fecha_horario = None
            try:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT tipo_fk, fecha FROM horarios_jornada WHERE id_pk = %s", [id_horario_int])
                    resultado = cursor.fetchone()
                    if resultado:
                        tipo_horario = resultado[0]
                        fecha_horario = resultado[1]
            except:
                pass
            ...
            with connection.cursor() as cursor:
                ...
                # ==============================================
                # BITÁCORA: Registrar eliminación de horario
                # ==============================================
                empleado_id = request.session.get('empleado_id')
                if empleado_id and BITACORA_DISPONIBLE and tipo_horario:
                    fecha_str = fecha_horario if fecha_horario else 'general'
                    registrar_crud_action(
                        request,
                        empleado_id,
                        'ELIMINAR',
                        'HORARIOS',
                        objeto_descripcion=f"Horario para {tipo_horario} ({fecha_str})"
                    )
    ...
    # ==============================================
    registros_por_pagina = 10
    paginator = Paginator(horarios_lista, registros_por_pagina)
    page = request.GET.get('page', 1)

    try:
        horarios_paginados = paginator.page(page)
    except PageNotAnInteger:
        horarios_paginados = paginator.page(1)
    except EmptyPage:
        horarios_paginados = paginator.page(paginator.num_pages)

    # Calcular información de paginación
    if horarios_paginados:
        inicio = (horarios_paginados.number - 1) * registros_por_pagina + 1
        fin = inicio + len(horarios_paginados.object_list) - 1
        rango_mostrado = f"{inicio}-{fin}"
    else:
        rango_mostrado = "0-0"

    total_registros = len(horarios_lista)
    total_paginas = paginator.num_pages
    
    context = {
        'page_title': 'Gestión de Horarios de Jornada',
        'menu_activo': 'seguridad',
        'horarios_lista': horarios_paginados,
        ...
        'total_registros': total_registros,
        'total_paginas': total_paginas,
        'pagina_actual': horarios_paginados.number,
        'rango_mostrado': rango_mostrado,
        'registros_por_pagina': registros_por_pagina,
    }
